...
import { updateBuyerSchema, UpdateBuyerInput } from '@/lib/validations';
...
export function BuyerEditForm({ buyer, canEdit }: BuyerEditFormProps) {
  ...
  const {
    ...
    resolver: zodResolver(updateBuyerSchema) as any,
    defaultValues: {
      id: buyer.id,
      ...
      notes: buyer.notes || '',
      tags: buyer.tags || [],
      updatedAt: Math.floor(buyer.updatedAt.getTime() / 1000),
    },
  });
  ...
  const onSubmit: SubmitHandler<UpdateBuyerInput> = async (data) => {
    if (!canEdit) {
      setSubmitError('You do not have permission to edit this buyer');
      return;
    }
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (response.status === 409) {
          throw new Error('This buyer has been modified by someone else. Please refresh and try again.');
        }
      ...
      setSubmitError(error instanceof Error ? error.message : 'Failed to update buyer');
    } finally {
    ...
    return (
      <div className="bg-white shadow rounded-lg p-6">
        <div className="text-center">
          <p className="text-gray-500">You do not have permission to edit this buyer.</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="bg-white shadow rounded-lg">
      <div className="px-6 py-4 border-b border-gray-200">
        <h1 className="text-xl font-semibold text-gray-900">Edit Buyer</h1>
      </div>
